...
| 現有設定 | 通過多重驗證的具名超級管理員可編輯有稽核的結帳受理限制，預設停用。連線準備狀態仍為唯讀，不揭露憑證。 | 驗證真實 Turnstile 設定與邊緣來源信任，核准配額、運費／地區、稅務及退款／聯絡政策。儲存受理限制不會解除結帳硬性門檻。 |
| 現有結帳原始碼 | Stripe 程式與 SQL 包含庫存保留、伺服器定價、歸屬、事件去重、到期及付款覆核。已實作 Turnstile 驗證、可信任邊緣 IP 的 HMAC 與原子用戶／全店配額；可選到期維護共用既有工作程序。 | 在獲授權的資料庫與付款環境執行遷移及完整測試。目前 `checkoutReleaseReady()` 回傳 `false`，正式結帳繼續關閉。 |
...
| 查看準備狀態／編輯結帳受理限制 | 可，須多重驗證 | 不可 | 不可 | 不可 | 不可 | 不可 |
...
7. **設定：**擁有人連線準備狀態仍為唯讀；通過多重驗證的擁有人可編輯有稽核的結帳受理配額，預設停用。儲存限制不會開放交易；商戶政策、物流及付款憑證仍在編輯器之外，啟用資料尚待確立。
...
已實作的受理流程要求伺服器驗證 Turnstile，綁定網站主機名稱、`checkout` 動作及請求識別碼，並檢查時效；驗證有八秒期限及回應大小上限。用戶範圍來自可信任 Cloudflare 邊緣 IP，正規化後以伺服器機密計算 HMAC；不接受瀏覽器識別碼或一般轉送標頭。私人紀錄僅保存雜湊、訂單與時間，驗證時會把原始位址傳給 Turnstile。共用位址共用配額，這不等於顧客身分，部署時須驗證標頭確由可信任邊緣提供。

受理預設停用。通過多重驗證的具名擁有人可調整並稽核限制：預設每個用戶十五分鐘內五次成功新受理、每個用戶兩筆有效預留訂單、全店一百筆。共用庫存交易鎖令配額檢查與預留具原子性。失敗預留不計額度；相同用戶／購物內容重試重用原訂單，不重複扣額度，即使新受理已暫停。舊無計量介面已關閉。設定介面啟用受理前須具備驗證設定，但存在設定不代表平台連線通過。`checkoutReleaseReady()` 的硬性門檻仍回傳 false。

可選的 `COMMERCE_MAINTENANCE_ENABLED=true` 在既有工作程序處理圖片前執行一次到期維護，預設停用，亦支援只執行維護。每次請求最多十秒，並受整體工作時間限制；未能確認清理會令本次執行失敗，以 `UNKNOWN` 及空值記錄釋放數量。尚未建立真實排程。SQL 清理為單一交易，並非分批資料列；用戶端逾時不代表資料庫已取消交易。受管資料庫／介面的敘述與鎖定逾時、最新維護健康狀態及真實到期／付款競爭，仍屬啟用門檻。詳見[交易準備狀態](COMMERCE-READINESS.md)及[工作程序操作文件](../batch_helper/deploy/RUNBOOK.md)。
...
儲存庫包含權限、來源／商品欄公開、遞增置頂、訂單範圍與狀態、批次租約／版本／容量及程序失敗路徑的應用及本機資料庫測試。此次發布收據記錄原始碼雜湊及最新結果；下列附日期數量是本機驗證快照。本機合成圖片基準不代表網路或正式環境吞吐量；本機單連線 PostgreSQL 測試不能證明多連線競爭。

上述測試未建立真實邀請寄達、有效員工登入／多重驗證、遠端遷移／儲存存取、已安裝工作程序、真實平台直播或商戶付款全流程的證據。設定須清楚區分可編輯的受理配額，以及尚未驗證的連線、商戶政策、物流與工作程序。正式啟用需要留下完整流程證據，真實結帳仍維持關閉，直到其獨立發布門檻通過。

9 月 25 日驗證報告記錄一百七十項應用測試及三十六項結帳受理 SQL 檢查。原生資料庫在可拋棄的 PostgreSQL 17.11 環境中，九種競爭情境各以八條連線測試，共通過十五項斷言：最後一件庫存預留、批次數量上限、保留儲存量上限、訂單版本衝突、訂單重試、結帳近期受理配額、用戶有效預留配額、全店有效預留配額及結帳重試。結果僅建立已測本機並行性質；真實 Turnstile／小工具啟用、受管 Supabase、到期排程、遠端敘述逾時、平台生命週期及真實帳戶測試仍待驗證。
