...
7. **Settings:** owner-only connection readiness remains read-only; the MFA-protected owner can edit audited checkout admission quotas, disabled by default. Saving limits does not enable commerce. Merchant policies, shipping settings and payment credentials remain outside this editor and unresolved for activation.
...
### Checkout admission and expiry maintenance

Implemented admission requires server-verified Turnstile bound to the site hostname, `checkout` action and attempt UUID, with freshness checks, an eight-second verification deadline and bounded response size. Client scope is a server-secret HMAC of normalized, trusted Cloudflare edge IP; browser IDs and generic forwarded headers are ignored. The private ledger holds only hash/order/time, while the address is sent to Turnstile for verification. Shared addresses share quotas; this is not customer identity, and deployment must verify trusted-header provenance.

Admission defaults to disabled. The named, MFA-verified owner may set audited limits: defaults are five new successful admissions per client in fifteen minutes, two active held orders per client and one hundred store-wide. A shared inventory transaction lock makes quota checks and stock reservation atomic. Failed reservations are not charged; exact same-client/cart retries reuse their order without another charge, even when new admissions are paused. The legacy unmetered RPC is closed. Verification configuration is required to enable admission through the settings API, but configuration alone proves no provider connection. The hard `checkoutReleaseReady()` gate still returns false.

Optional `COMMERCE_MAINTENANCE_ENABLED=true` runs expiry once before image jobs in the existing worker; it defaults to disabled and supports maintenance-only runs. A ten-second request deadline is capped by the worker budget; an unconfirmed sweep fails the run and records `UNKNOWN` with a null released count. No live scheduler is established. The SQL sweep is one transaction, not row-batched; a client timeout does not prove database cancellation. Managed database/API statement and lock timeouts, fresh sweep health and real expiry-versus-payment behavior remain activation gates. See [commerce readiness](COMMERCE-READINESS.md) and the [worker runbook](../batch_helper/deploy/RUNBOOK.md).
...
The repository includes focused application and local database tests for permissions, source/rail publication, monotonic pins, order scope and transitions, batch leases/revisions/capacity and worker failure paths. The owning release receipt records source hashes and the latest results; the dated counts below are a local verification snapshot. A local synthetic-image benchmark does not measure network or production throughput. Local single-connection PostgreSQL tests do not prove multi-connection races.

No real invitation delivery, active staff login/MFA, remote migration/storage access, installed worker, actual provider broadcast or merchant payment lifecycle is established by these tests. Settings must continue to distinguish editable admission quotas from unverified connection, merchant-policy, shipping and worker evidence. Production activation requires recorded end-to-end proof; real checkout remains disabled until its independent release gate is satisfied.

The 25 September verification report records 170 application tests and 36 checkout-admission SQL checks. Native database follow-up passed 15 assertions on disposable PostgreSQL 17.11 across nine contention scenarios with eight connections each: last-unit reservation, batch count cap, retained storage cap, order revision conflicts, order retries, checkout recent-attempt quota, client active quota, store active quota and checkout retries. This establishes only the tested local concurrency properties. Real Turnstile/widget activation, managed Supabase, expiry scheduling, remote statement timeouts, provider lifecycle and real account tests remain unverified.
