...
The initial showroom import preserved 96 demo products and images, three fictional hosts, three public preview rooms and eighteen product links. The subsequent explicit-demo batch migration is applied and production batch access enabled. Owner sign-in and verified TOTP MFA are confirmed. One demo item completed authenticated upload, review and approval, with processing/publication performed by bounded manual worker passes, bringing the catalogue to 97 demo products. Thirty-one read-only publication checks passed, including service-verified original/output hashes, denial of unsigned public access to both private objects, an approved public derivative with the exact output hash, `is_demo=true`, stock zero, reviewed bilingual copy and zero orders.

This proves the bounded one-item workflow, not ordinary authenticated-user storage isolation, thousand-image throughput, real merchant approval or verified broadcasts. The named image worker was activated at 10:30:21 UTC on 26 September (60-second interval, 20-job limit, 45-second budget). Two automatic idle cycles passed with zero completed/failed jobs and released locks; database counts remained unchanged, including zero orders/reservations. This proves idle scheduling only; a nonempty scheduled workload and restart/recovery remain unverified. Inventory-expiry maintenance stays disabled. No owner credentials, factor secrets or private identity details are published.

The earlier connected release passed 184 application checks; the explicit-demo release passed 186 application tests and 143 connected HTTP checks. These bounded results establish the checked pages and guarded endpoints, not all managed database, Auth, storage or payment behavior. Recovery rehearsal, real provider/widget verification, payment merchant/lifecycle tests and the independent checkout gate remain open requirements. See [managed deployment](../supabase/DEPLOYMENT.md).
...
- Complete the remaining managed-backend workflow checks after the verified migration/RLS deployment: ordinary authenticated-user private storage isolation, other staff/KOL scopes, nonempty scheduled-workload, restart and large-batch behavior beyond the manual one-item owner workflow and two automatic idle cycles, backup/recovery rehearsal, and independent-connection last-unit/expiry-versus-payment tests on the approved project.
