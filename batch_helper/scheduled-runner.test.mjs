...
test('scheduled runner is single-owner, bounded, and exposes stale health without secrets',async()=>{
 const server=createServer((_request,response)=>{response.writeHead(200,{'content-type':'application/json'});response.end('[]');});await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));
 ...
 const env={...process.env,SUPABASE_URL:`http://127.0.0.1:${server.address().port}`,SUPABASE_SERVICE_ROLE_KEY:'fixture-secret-must-not-leak',BATCH_HELPER_ENABLED:'true'};
