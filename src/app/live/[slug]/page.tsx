...
import { getLiveSessionBySlug, getLiveSessions } from "@/lib/data/store";
...
export default async function LiveDetailPage({params}:{params:Promise<{slug:string}>}) {
 ...
 const others=(await getLiveSessions()).filter(s=>s.id!==session.id);
 ...
 return <div className="live-page"><div className="shell live-breadcrumb"><Link href="/live">← All rooms · 全部直播间</Link><span>{session.platform.toUpperCase()}</span></div>
  <div className="shell watch-shop"><section className="watch-column"><LivePlayer session={session}/><div className="room-description"><LiveBadge status={session.status}/><h1>{session.title}</h1><p>{session.description}</p><div className="host-profile"><span className="host-avatar" aria-hidden="true">{session.hostName.slice(0,1)}</span><div><strong>{session.kol?.displayName ?? session.hostName}</strong><p>{session.kol?.bio}</p></div></div>{session.status!=="preview" && <a className="text-link" href={session.externalUrl} target="_blank" rel="noopener noreferrer">Watch on {session.platform} · 原平台观看 ↗</a>}</div></section>
  <aside className="room-products"><div className="section-heading"><div><span className="eyebrow">IN THIS ROOM</span><h2>Shop the selection<br/>本场精选</h2></div><Link className="text-link" href="/cart">Bag · 购物袋 →</Link></div>{session.products.length?<div className="room-product-grid">{session.products.map(p=><ProductCard key={p.id} product={p} source={source}/>)}</div>:<p className="empty-inline">Products are being prepared · 商品准备中</p>}<p className="secure-note">Selections keep their room source in your bag. 商品保留主播来源。</p></aside></div>
